...
import './App.css';  // Assuming your CSS file is in the src directory
...
const TicTacToe = () => {
  ...
  const handleBoxClick = (number) => {
    if (winner || playerOneChoices.includes(number) || playerTwoChoices.includes(number)) return;

    const newPlayerChoices =
      turn === 'X' ? [...playerOneChoices, number] : [...playerTwoChoices, number];
    ...
    <main>
      <div className="top">
        ...
        <section className="turn">
          <h3 id="colorChange">
            <i className={`fa-solid ${turn === 'X' ? 'fa-x' : 'fa-o'}`} id="turn"></i> TURN
          </h3>
        </section>
        <div>
          <button className="clearScore" id="clear-score" onClick={clearScores}>Wipe Score</button>
        </div>
      </div>
      ...
      <div className={`winner ${winner ? 'show-winner' : ''}`} id="winner-declaration">
        <h4 id="winner-text">{winner && `The winner is ${winner}`}</h4>
        <button className="restart-after-win" id="restart-after-win" onClick={restartGame}>
          Play again :)
        </button>
      </div>
    </main>
